...
def main : IO Unit := do
  ...
  let line := πx ∧ πy
  IO.println s!"\nx=0 ∧ y=0  = {showp line}   (the z-axis as a bivector)"

  let pt := line ∧ πz
  IO.println s!"x=0 ∧ y=0 ∧ z=0 = {showp pt}  (the origin as a trivector)"

  IO.println "\npoint (1,2,3) constructed from intersecting planes:"
  let P := point 1.0 2.0 3.0
  IO.println s!"  P = {showp P}"

  IO.println "\ntranslator T = 1 - ½ (dx e01 + dy e02 + dz e03), apply via sandwich"
  let T := translator 10.0 0.0 0.0
  IO.println s!"  T = {showp T}"
  let P' := applyMotor T P
  IO.println s!"  T·P·T~ = {showp P'}"

  IO.println "\nT² = translation by (20,0,0) (translators are exp of null bivectors)"
  let T2 := T.gp T
